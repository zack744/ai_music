# ai_music · 从"环境音 + 一句话"到一首成曲

录两段音频（一段环境音乐 + 一段你说话），系统自动完成理解、过滤、增强提示词、生成，返回一首完整的歌。
...
- **v1 黑盒**：只有"录音 / 下一步 / 生成"3 个按钮。跑通了，但交互性太差，覆盖不了通用场景。
- **v2 加 UI**：给 ESP32 做触摸屏界面。这段卡最久——屏幕驱动固件官方文档冷门、网上方案少，最后找同类型仓库 + 官方文档，让 AI 理解后写通。
- **v3 两段音频工作流**：环境音和人声分开录，先识别理解，再用通用 LLM 过滤无效提示词、增强提示词，最后交给音乐生成模型。需求明确后才补的 UI。
